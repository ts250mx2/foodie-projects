...
interface MobileBatchModalProps {
    ...
    onClose: () => void;
    projectId: number;
    /** Called when user clicks "Procesar como Gasto" */
    onProcessAsExpense: (photos: BatchSelectedPhoto[]) => void;
    /** Called when user clicks "Procesar como Compra" */
...
export default function MobileBatchModal({
    ...
    const openBatch = async (batch: BatchCard) => {
        ...
        try {
            ...
            // 2. Load photos (base64) for each detail
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-slate-100">
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {batches.map(batch => {
                                    ...
                                    const allProcessed = batch.FotosPendientes === 0 && batch.TotalFotos > 0;
                                    return (
                                        <button
                                        ...
                                        >
                                            <div className="flex gap-4 items-start">
                                                ...
                                                <div className="w-16 h-16 rounded-2xl overflow-hidden bg-slate-100 flex-shrink-0 border border-slate-100">
                                                    {batch.FirstThumb ? (
                                                        <img
                                                            src={`data:image/jpeg;base64,${batch.FirstThumb}`}
                                                            alt=""
                        ...
                        <div className="flex flex-col items-center justify-center py-8 gap-6">
                            ...
                            <div className="text-center max-w-xs">
                                ...
                                <p className="text-xs text-slate-400 leading-relaxed">
                                    Apunta la cámara de tu celular al QR. Se abrirá una página donde podrás tomar fotos de recibos directamente. Cada foto se guardará automáticamente.
                                </p>
                            </div>
                            {qrUrl && (
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`relative aspect-square rounded-3xl overflow-hidden border-4 transition-all ${
                                                        isProcessed ? 'border-green-200 opacity-60 cursor-not-allowed' :
                                                        isSelected ? 'border-indigo-500 scale-[1.02] cursor-pointer shadow-lg shadow-indigo-100' :
                                                ...
                                                >
                                                    ...
                                                    {/* Selection indicator */}
                                                    {!isProcessed && (
                                                        <div className={`absolute top-2 left-2 w-7 h-7 rounded-full flex items-center justify-center z-10 transition-all ${
                ...
                {/* Footer Actions (only in detail view with selection) */}
