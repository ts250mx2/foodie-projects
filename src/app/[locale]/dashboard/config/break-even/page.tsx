...
export default function BreakEvenPage() {
    ...
    const avgSalesAmount = avgTicket * volume;
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (data.success && data.data) {
                setAvgTicket(data.data.PrecioTicket || 0);
                setVolume(data.data.VolumenTickets || 0);
                ...
                setAvgTicket(0); setVolume(0); setRawMaterial(0); setPackaging(0); setOthers(0); setShipping(0); setFixedExpenses([]);
    ...
    return (
        <div className="flex flex-col min-h-screen p-4 gap-4 bg-slate-50/50">
            ...
            <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-6 pb-8">
                ...
                {/* COLUMN LEFT (4 cols) - BASIC BLOCKS */}
                    ...
                    <div className="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden group hover:shadow-lg transition-all">
                        ...
                        <div className="p-5 space-y-4">
                            <div className="flex items-center justify-between gap-4 p-2 rounded-xl hover:bg-slate-50 transition-colors">
                                <span className="text-[11px] font-black text-slate-500 uppercase flex-1">{t('avgTicketPrice')}</span>
                                <div className="relative w-28">
                                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] font-bold">$</span>
                                    <input type="number" value={avgTicket || ''} onChange={e => setAvgTicket(parseFloat(e.target.value) || 0)} className="w-full pl-5 pr-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg outline-none text-xs font-black text-slate-800 text-right focus:ring-2 focus:ring-indigo-500/20" />
                                </div>
                            </div>
                            <div className="flex items-center justify-between gap-4 p-2 rounded-xl hover:bg-slate-50 transition-colors">
                                <span className="text-[11px] font-black text-slate-500 uppercase flex-1">{t('ticketVolume')}</span>
                                <div className="relative w-28">
                                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] font-bold">#</span>
                                    <input type="number" value={volume || ''} onChange={e => setVolume(parseFloat(e.target.value) || 0)} className="w-full pl-5 pr-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg outline-none text-xs font-black text-slate-800 text-right focus:ring-2 focus:ring-indigo-500/20" />
                                </div>
                            </div>
                            <div className="flex items-center justify-between gap-4 p-3.5 bg-emerald-50 rounded-xl border border-emerald-100 mt-2">
                                <span className="text-[10px] font-black text-emerald-800 uppercase tracking-wider">{t('avgSalesAmount')}</span>
                                <span className="text-base font-black text-emerald-600">{formatCurrency(avgSalesAmount)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden group hover:shadow-lg transition-all">
                        ...
                        <div className="p-5 space-y-2">
                            {[
                                ...
                                <div key={item.k} className="flex items-center justify-between gap-4 p-1.5 rounded-lg hover:bg-slate-50 transition-colors">
                                    <span className="text-[11px] font-black text-slate-500 uppercase flex-1">{t(item.k)}</span>
                                    <div className="relative w-28">
                                        <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] font-bold">$</span>
                                        <input type="number" value={item.v || ''} onChange={e => item.s(parseFloat(e.target.value) || 0)} className="w-full pl-5 pr-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg outline-none text-xs font-black text-slate-800 text-right focus:ring-2 focus:ring-orange-500/20" />
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="p-5 space-y-5">
                            <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                                {fixedExpenses.map((exp, idx) => (
                                    <div key={idx} className="flex items-center gap-2 p-1.5 bg-slate-50/50 rounded-lg border border-slate-100 group">
                                        <input
                                            ...
                                            className="flex-1 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-200 shadow-sm"
                                        />
                                        <div className="relative w-28">
                                            <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-slate-300 text-[10px] font-bold">$</span>
                                            <input type="number" value={exp.Monto || ''} onChange={e => handleUpdateExpense(idx, 'Monto', parseFloat(e.target.value) || 0)} className="w-full pl-5 pr-2 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 text-right outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-200 shadow-sm" />
                                        </div>
                                        <button onClick={() => handleDeleteExpense(idx)} className="p-1 px-1.5 text-rose-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all">🗑️</button>
                                    </div>
                                ))}
                            </div>
                ...
                {/* COLUMN RIGHT (8 cols) - ANALYSIS & CHART */}
                            ...
                            <table className="w-full text-left border-separate border-spacing-0">
                                ...
                                <tbody className="text-[11px] font-bold text-slate-600">
                                    <tr className="border-t border-slate-50">
                                        ...
                                        {scenarios.map((s, i) => (
                                            <td key={i} className="p-1.5 border-x border-slate-50">
                                                <input type="number" value={s.PrecioTicket || ''} onChange={e => handleUpdateScenario(i, 'PrecioTicket', parseFloat(e.target.value) || 0)} className="w-full bg-white border border-slate-200 rounded-lg p-2 text-right outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-sm" />
                                            </td>
                                        ))}
                                    </tr>
                                    <tr className="border-t border-slate-50">
                                        ...
                                        {scenarios.map((s, i) => (
                                            <td key={i} className="p-1.5 border-x border-slate-50">
                                                <input type="number" value={s.VolumenTickets || ''} onChange={e => handleUpdateScenario(i, 'VolumenTickets', parseFloat(e.target.value) || 0)} className="w-full bg-white border border-slate-200 rounded-lg p-2 text-right outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-sm" />
                                            </td>
                                        ))}
                                    </tr>
