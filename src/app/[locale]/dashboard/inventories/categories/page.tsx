...
export default function CategoriesPage() {
    ...
    return (
        <PageShell title={t('title')} subtitle={`${categories.length} categorías registradas`} icon={FolderOpen} actions={
                    ...
                    {!isLoading && sortedAndFilteredCategories.length > 0 && (
                        <div className="px-6 py-3 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between">
                            <span className="text-xs text-gray-600 font-medium">
                                {sortedAndFilteredCategories.length} de {categories.length} categorías
                            </span>
                        </div>
                    )}
                </div>
