/**
 * Cliente de HL Console (proyecto `hl-servidor`): de ahí salen el proveedor y
 * el modelo de la IA de Foodie, y por ahí pasan todas las llamadas al
 * proveedor. Del .env solo se leen los datos para hablar con HL:
 *   HL_URL             = http://127.0.0.1:3056  (sin diagonal final; HL sirve HTTP plano)
 *   HL_API_KEY         = hl_ + 48 hex           (key de acceso de esta app)
 *   HL_AGENTE_FOODIE   = UUID del agente Foodie (chat web, WhatsApp, reportes, OCR)
 *   HL_AGENTE_RESPALDO = UUID del agente de respaldo (opcional: si el proveedor
 *                        del agente falla, el turno se repite con él)
 ...
 * solo agente, HL_AGENTE, con un solo cache) siguiendo la misma estructura que
 * vidaurri-ia/src/lib/hl-cliente.ts, donde cada agente tiene su UUID y su
 * propio cache. La key de acceso se llama HL_API_KEY en esta app.
 *
 * Si cambias el modelo o rotas la llave en el portal, se toma al vencer el
 * cache (HL_TTL_MIN) o al llamar limpiarCacheAgentes().
 */

/** foodie es el agente; respaldo es la credencial a la que cae si su proveedor falla. */
export type AgenteHl = 'foodie' | 'respaldo';

const VARIABLE_AGENTE: Readonly<Record<AgenteHl, string>> = {
    foodie: 'HL_AGENTE_FOODIE',
...
export interface AgenteIA {
    uuid: string;
    /** Nombre del agente en el portal ("Agente Foodie"). */
    nombre: string;
...
/** Por UUID del agente: Foodie y el respaldo nunca comparten entrada. */
